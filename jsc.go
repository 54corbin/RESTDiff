...
func order(json2 *[]interface{}, json1 *[]interface{}) {
	...
	moreArr := make([]string, len(*more))
	for i, m := range *more {
		moreArr[i] = fmt.Sprintf("%+v", m)
	}
	for i, le := range *less {

		sim := strsim.FindBestMatch(fmt.Sprintf("%+v", le), moreArr)
		// fmt.Printf("sim:%+v\n", sim)

		tmp := (*more)[i]
		(*more)[i] = (*more)[sim.BestIndex]
		(*more)[sim.BestIndex] = tmp
		moreArr[sim.BestIndex] = ""
	}

	// fmt.Printf("vvvvvvvvvv\nless->%+v\nmore->%+v\n", less, more)
...
func LoadJsonFromString(jsonStr string, dist interface{}) (err error) {

	return json.Unmarshal([]byte(jsonStr), dist)
}

func LoadJsonFromFile(path string, dist interface{}) (err error) {
	...
	rjstr := `
{
 ...
 "cc":[
	...
	lj := make(map[string]interface{})
	rj := make(map[string]interface{})

	if err := LoadJsonFromString(ljstr, &lj); nil != err {
		panic(err)
	}
	if err := LoadJsonFromString(rjstr, &rj); nil != err {
		panic(err)
	}

	diff, b := JsonCompare(lj, rj, -1)
	if b {
		fmt.Println(diff)
	}
